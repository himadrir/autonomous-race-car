...
def laser_callback(lidar_msg):
    ...
    d = b_ * cos(alpha)
    # d_ = d + 1.00 * sin(alpha) #replace 1 with curr velocity read from the topic /curr_vel * >
...
def pid_controller(error, d):
    ...
    t_print_debug = rospy.Time.now().to_sec()
    dt = t_now - prev_time
   # integral += error * dt
    try:
...
if __name__ == '__main__':
    ...
    try:
        ...
        cmd_vel_pub = rospy.Publisher("/wf_drive", AckermannDriveStamped, queue_size=1)
